...
class PointVector(Matrix):
    ...
    #     """
    #     return Matrix( param ) #super(Matrix,cls).__new__(cls,param)
    # def __init__(self, param):
    #     super().col_op(0, lambda v, i: param[i])
#        super().col_op(0, lambda v, i: 6)
    ...
    def tolist(self):
        """Returns the PointVector converted in a python list"""
        return [ x[0] for x in super().tolist() ]
    ...
    def norm(self, type):
        ...
        """
        if type == "L1":
            return sum([ abs(x) for x in self.tolist() ])
        elif type == "L2":
            return math.sqrt(sum([ x**2 for x in self.tolist() ]))
        elif type == "Linf":
            return max([ abs(x) for x in self.tolist() ])
        else:
            raise ValueError
