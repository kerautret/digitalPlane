...
\dav{Starting from raw normal vector estimations (\emph{e.g.} unit
  ...
  \cite{francon,Fourey2009,Esbelin2011,Esbelin2016} with the following
  drawbacks: the user must specify the kernel parameters and size, and
  the smoothing is usually isotropic and may smooth out singularities
  (or features) of the surface. However, if the kernel size is a function of
  the  gridstep, multigrid convergence can be obtained \cite{REFF}. We
also mention mesh denoising techniques, which also output a smooth normal vector
field, based on more advanced local filters such as the bilateral one
...
\dav{\noindent\textbf{Variational approaches}.  We conclude this
  ...
  we are losing pointwise multigrid convergence.}
