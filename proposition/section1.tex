...
\dav{Thanks to non-invasive acquisition devices such as magnetic
  resonance, X-ray tomography or micro-tomography, processing the
  geometry of subsets or partitions in volumetric images is crucial in
  many applications such as Material Sciences or Medical Imaging
  (\emph{e.g}
  \cite{Hildebrand1999,dcoeurjo_flin_ImPro,dardenne2009variational}). Many
  volumetric acquisition devices generate regularly spaced data and we
  have to process the geometry of regions defined as subsets in these
...
the grid step. \dav{In dimension 2, multigrid convergent
...
\dav{Starting from raw normal vector estimations (\emph{e.g.} unit
  vectors perpendicular to surfels), several approaches refine the
  estimation by convoluting of the raw normals using a given smoothing
  kernel. On digital surfaces, Gaussian or isotropic kernel in ambient
  space have been considered
  \cite{papierthese,Fourey2009,Esbelin2011,Esbelin2016} with the
  following drawbacks: the user must specify the kernel parameters and
  size, and the smoothing is usually isotropic and may smooth out
  singularities (or features) of the surface. However, if the kernel
  size is a function of the gridstep, multigrid convergence can be
  obtained in 2-D \cite{Esbelin2011}. We also mention mesh denoising
  techniques, which also output a smooth normal vector field, based on
  more advanced local filters such as the bilateral one leading to an
  anisotropic smoothing of the normal vector field
  \cite{fleishman2003bilateral,zheng2011bilateral}. }
...
\dav{\noindent\textbf{Variational approaches}.  We conclude this
